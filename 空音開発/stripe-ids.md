...
### Phase 5c 追加: 教師経由 学生クーポン (2026-04-28)

| Coupon ID | percent_off | duration | applies_to | 顧客支払 (月額) | 月次純利 |
|-----------|------------|----------|-----------|---------|---------|
| `STUDENT_30_12MO` | 30% | repeating (12 ヶ月) | Prelude + Concerto | Prelude ¥546 / Concerto ¥1,036 | Prelude +¥476 / Concerto +¥749 |

**設計意図**:
- 知り合いの音楽家・教師に Symphony を無償付与し、その学生向けに教師の名前を冠した Promotion Code を発行 (例: `TANAKA-30`)
- Symphony は格式維持のため除外。Prelude / Concerto のみ
- 12 ヶ月後 → 自動的に通常価格 (Stripe `duration: repeating, duration_in_months: 12` で実現)
- 年額に適用すると「2 ヶ月無料 + 30% off」になるが、年額契約者の LTV 価値で十分相殺
- `restrictions.first_time_transaction: true` で既存課金者の二重利用防止

**運用 UI**:
- `/admin/coupons` で教師ごとに Promotion Code を発行・無効化・統計表示
- Worker エンドポイント: `POST/GET/PATCH /api/auth/admin/promo-code(s)`
- 教師には `kuon-rnd.com/?coupon=TANAKA-30` 形式の共有 URL を渡す（自動適用）

**コード参照**:
- `kuon-rnd-auth-worker/src/stripe-prices.ts` の `COUPON_IDS.student_30_12mo`
- `kuon-rnd-auth-worker/src/index.ts` の `/api/auth/admin/promo-code` 系エンドポイント
- `app/admin/coupons/page.tsx` の発行 UI
