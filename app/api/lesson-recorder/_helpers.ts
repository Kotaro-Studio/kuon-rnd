/**
 * KUON LESSON RECORDER — 共通プロキシヘルパー
 *
 * フロント → Auth Worker (JWT 検証 + plan 取得) → Recorder Worker (実処理)
 */
...
/**
 * 認証情報を取得 (JWT cookie or Authorization header)
 */
...
/**
 * クォータを track (+1) する。失敗時は 429/403 を返す。
 */
...
/**
 * Recorder Worker への共通呼び出し (Bearer + ユーザーヘッダー付加)
 */
export async function callRecorderWorker(
  user: AuthedUser,
  path: string,
  init: RequestInit = {},
): Promise<Response> {
  const url = process.env.RECORDER_URL || 'https://kuon-rnd-recorder-worker.369-1d5.workers.dev';
