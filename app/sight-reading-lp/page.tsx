...
export default function SightReadingLP() {
  ...
  const modes = [
    ...
    { icon: '🔄', title: { ja: '記譜変換 (Notation Conversion)', en: 'Notation Conversion', ko: '기보 변환', pt: 'Conversao de Notacao', es: 'Conversion de Notacion' } as L5, desc: { ja: 'ドイツ音名・英語音名・日本音名の3体系を自在に変換する。「H-dur は何調？」「Fis は英語で？」「変ロ短調のドイツ語名は？」— 楽典試験の記譜変換問題を完全対策。', en: 'Convert freely between German, English, and Japanese notation systems. "What key is H-dur?" "What is Fis in English?" — Master the notation conversion questions that appear on music theory exams.', ko: '독일 음명·영어 음명·일본 음명 3체계를 자유자재로 변환. "H-dur는 무슨 조?" "Fis는 영어로?" — 악전 시험의 기보 변환 문제를 완벽 대비.', pt: 'Converta livremente entre os sistemas de notacao alemao, ingles e japones. "Que tom e H-dur?" "O que e Fis em ingles?" — Domine as questoes de conversao de notacao dos exames.', es: 'Convierte libremente entre los sistemas de notacion aleman, ingles y japones. "Que tono es H-dur?" "Que es Fis en ingles?" — Domina las preguntas de conversion de notacion de los examenes.' } as L5 },
  ];
  ...
  const examBenefits = [
    ...
    { num: '05', text: { ja: 'ドイツ音名 ↔ 日本音名の変換が瞬時にできるようになる', en: 'Instantly convert between German and Japanese notation systems', ko: '독일 음명 ↔ 일본 음명 변환이 순식간에 가능해진다', pt: 'Converta instantaneamente entre notacoes alema e japonesa', es: 'Convierte instantaneamente entre notacion alemana y japonesa' } as L5 },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: sans, color: '#1e293b' }}>
      ...
      <section ref={addReveal} style={{ ...revealStyle, padding: 'clamp(40px, 8vw, 80px) clamp(16px, 4vw, 32px)', maxWidth: 700, margin: '0 auto' }}>
        <h2 style={{ fontFamily: serif, fontSize: 'clamp(18px, 4.5vw, 28px)', textAlign: 'center', marginBottom: 12 }}>
          {t({ ja: 'ドイツ音名・英語音名・日本音名。\n3つの記譜体系に完全対応。', en: 'German, English, and Japanese notation.\nFull support for 3 notation systems.', ko: '독일 음명·영어 음명·일본 음명.\n3가지 기보 체계 완벽 대응.', pt: 'Notacao alema, inglesa e japonesa.\nSuporte completo para 3 sistemas.', es: 'Notacion alemana, inglesa y japonesa.\nSoporte completo para 3 sistemas.' }, lang)}
        </h2>
        <p style={{ textAlign: 'center', color: '#64748b', fontSize: 'clamp(13px, 2.8vw, 15px)', marginBottom: 28, maxWidth: 560, margin: '0 auto 28px' }}>
          {t({ ja: 'クラシック音楽の国際標準であるドイツ音名をデフォルトに、ジャズ・ポップスで使われる英語音名、楽典試験に必要な日本音名（イロハ）の3体系を切り替えて学習できます。UI言語（日/英/韓/葡/西）とは独立して設定可能。', en: 'German notation (the international classical standard) is the default. Switch to English notation (jazz/pop) or Japanese notation (イロハ, for 楽典 exams). The notation system is independent of the UI language setting.', ko: '클래식 음악의 국제 표준인 독일 음명을 기본으로, 재즈·팝에서 사용되는 영어 음명, 악전 시험에 필요한 일본 음명(이로하) 3체계를 전환하며 학습할 수 있습니다. UI 언어(일/영/한/포/서)와 독립적으로 설정 가능.', pt: 'A notacao alema (padrao classico internacional) e o padrao. Alterne para notacao inglesa (jazz/pop) ou japonesa (para exames). O sistema de notacao e independente do idioma da interface.', es: 'La notacion alemana (estandar clasico internacional) es el predeterminado. Cambia a notacion inglesa (jazz/pop) o japonesa (para examenes). El sistema de notacion es independiente del idioma de la interfaz.' }, lang)}
        </p>
        <div style={{ display: 'grid', gap: 12 }}>
          {[
            ...
            { sys: '日本音名', example: 'ハ  嬰ハ  ニ  変ホ  ホ  ヘ  嬰ヘ  ト  変イ  イ  変ロ  ロ', note: { ja: 'イロハ式。楽典試験の必須知識。「嬰」= ♯、「変」= ♭。', en: 'Iroha system. Essential for 楽典 (gakuten) exams. 嬰 = ♯, 変 = ♭.', ko: '이로하식. 악전 시험 필수 지식. 嬰 = ♯, 変 = ♭.', pt: 'Sistema Iroha. Essencial para exames de teoria musical japonesa.', es: 'Sistema Iroha. Esencial para examenes de teoria musical japonesa.' } as L5, color: RED },
          ].map((item, i) => (
            <div key={i} style={{
