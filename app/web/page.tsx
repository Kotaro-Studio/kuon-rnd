...
const introduction = {
  ...
  text: (
    <>
      オーディオの世界において、不要な回路を通ることは音の純度を下げる「ノイズ」となります。<br />
      Webシステムの世界もまた、全く同じです。<br />
      <br />
      従来のWordPressに代表されるモノリス（一枚岩）なアーキテクチャは、アクセスのたびに<br />
      データベースを構築し直し、不要なレンダリング処理という「ノイズ」を発生させてきました。<br />
      空音開発では、フロントエンドとバックエンドを完全に分離する「Headless CMS」を採用。<br />
      <br />
      そして、ユーザーの最も近いネットワークの境界（エッジ）でコンテンツを処理・配信する<br />
      「エッジコンピューティング（Edge Computing）」を統合しています。<br />
      これにより、サーバーのオーバーヘッドを極限まで削ぎ落とし、ミリ秒単位の超高速表示と、<br />
