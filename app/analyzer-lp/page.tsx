...
const T = {
  ...
  valueBody: {
    ja: 'LUFS メーターを無料で使いたい — そう思って検索した方も多いでしょう。\n\nKUON ANALYZER は、その期待に応えるだけでなく、もうひとつの武器を提供します：スペクトラムアナライザーとリファレンス比較機能です。\n\nプロのミキサーやマスタリングエンジニアは、必ずリファレンス楽曲と自分のミックスを比較しています。iZotope や FabFilter のプラグインを使って。それが今、ブラウザだけで、無料でできます。\n\n空音開発のツールは「分析して終わり」ではありません。問題を見つけたら、そのまま MASTER CHECK で修正 → 最適化済み WAV をダウンロード。分析から修正までが一気通貫です。',
