...
- 月額タイムライン (月 1 / 月 2-13 / 月 14 の 3 段階)
- 年額タイムライン (年 1 / 年 2 の 2 段階)
- 12 ヶ月顧客支払い試算表 (Prelude / Concerto × 月額・年額の 4 行)
- 教師への説明テンプレート (コピペで使える文面)
- 技術的詳細 (折りたたみ details: エッジケース 6 件)

### 44.9 教師向け説明 LP `/for-teachers` (隠しページ)

オーナーが教師営業時にミーティング前後に共有する事前案内 LP。教師が IQ 90-105 でも完璧に理解できる構成。

#### 隠し設計（厳守）

- **本番サイトからリンク禁止**：header / footer / sitemap.ts / 公開 LP に一切リンクしない
- **検索エンジン非掲載**：`robots.noindex/nofollow/noarchive/nosnippet/noimageindex/notranslate` 全て設定済み（`app/for-teachers/layout.tsx`）
- **パスワードゲート**：`kuon`（クライアント側 localStorage で persistent ゲート）
- **唯一のアクセス経路**：`/admin/coupons` のヘッダーカードからのリンクのみ

#### 内容構成（8 セクション）

1. ヒーロー: 「あなたの学生に、最高の学割を。」
2. 空音開発について（簡単紹介・1 段落）
3. 先生が手に入れるもの（Symphony 無償提供 + 専用学生コード）
4. 学生が受ける割引・**月払いの場合**（3 段階タイムライン + 試算表）
5. 学生が受ける割引・**年払いの場合**（2 段階タイムライン + 試算表 + 「年払いが最もお得」の説明）
6. 使い方（3 ステップ：URL 受領 → 学生に転送 → 自動適用）
7. なぜこの制度を作ったのか（オーナーの音大時代のストーリー）
8. FAQ 8 問（Symphony 無償の本当性 / 12 ヶ月割引の挙動 / 解約時の扱い / 既存ユーザー利用不可 / 紹介報酬なし etc.）
9. 次のアクション（オーナー連絡先のみ・代理購入や決済は無し）

#### デザイン方針（教師向け）

- 明朝体ヘッダー + 余白多め（楽譜のような優雅さ）
- 専門用語禁止：「subscription」→「月額」、「promotion code」→「割引コード」
- 価格は大きく目立つ（テーブル + 色分け）
- タイムラインで時系列を視覚化（月 1 / 月 2-13 / 月 14 を矢印接続）
- 営業前送付前提なので「次にすること」セクションでオーナー連絡先のみ提示

#### 関連ファイル

- `app/for-teachers/page.tsx` — LP 本体（パスワードゲート + 8 セクション）
- `app/for-teachers/layout.tsx` — noindex メタデータ
- `app/admin/coupons/page.tsx` — 隠しページへのリンクカード（営業時の起点）

#### 運用フロー

```
1. オーナーが教師にミーティング依頼
2. 事前 or ミーティング中に /for-teachers の URL とパスワード "kuon" を教師に送付
3. 教師が LP を読む → 「興味あります」と返事
4. オーナーが /admin で教師アカウントを Symphony 切替
5. オーナーが /admin/coupons から教師専用コード発行
6. オーナーが共有 URL とコードを教師に送付
7. 教師が学生に共有 URL を配布
...
## 45. 【最重要・鉄の掟】教師経由学生クーポン制度は完全機密（2026-04-28 確定）

> このセクションは他のすべてのルールを上書きする最優先事項。
> 違反は信頼関係への重大な損害となるため、絶対に守ること。

### 45.1 鉄の掟の本文

**以下の事実は、オーナーと Claude だけの秘密である。**

- 教師は Symphony プランを永久無償で利用できる
- 学生は教師経由のクーポンで初月 50% off + 12 ヶ月 30% off を受けられる
- そもそも教師経由学生クーポン制度というものが存在する
- 制度の URL `/for-teachers` は隠しページ・パスワード `kuon`

**これらは公開 LP・FAQ・JSON-LD・OGP・SNS・どこにも漏らしてはならない。**

### 45.2 なぜ秘密なのか（音楽家の心理）

オーナー曰く：「こういうのが音楽家は一番嫌います」。理由：

1. **教師の威信を守る**：教師から学生に渡される割引コードは「先生が私のために特別に取ってくれた」と感じてもらうことに価値がある。「公式に誰でも 30% off」になると、教師の取り計らいが無価値になる
2. **学生間の不公平感を生まない**：教師経由でない一般加入者が「学生だけ 30% off？俺は通常価格？」と感じる構造は崩壊を招く
3. **音楽コミュニティの "察し合い" の文化**：日本の音楽教育文化では、表向きは平等・裏では教師が学生を支援する暗黙の構造がある。Kuon はこの文化に乗る
4. **§37.7 の沈黙の勝利戦略**：割引制度を表に出さないことで「なぜか音大生に Kuon ユーザーが多い」という結果だけが市場に伝わる構造

### 45.3 公開してはいけないキーワード一覧（NG ワード）

以下のキーワードを公開ファイル（`app/**/*.tsx` で `/admin/*` `/for-teachers` 以外、特に `*-lp/` 配下、`page.tsx` ホームページ、JSON-LD、OGP description）に書いてはいけない:

- `STUDENT_30_12MO`
- `教師のクーポン` `teacher coupon` `cupón de profesor` `cupom de professor` `Lehrkraft-Code` `교사 쿠폰`
- `教師は ... 無償/無料` `teachers ... free` `Symphony 永久無償` `Symphony free forever`
- `30% off × 12 ヶ月` `30% off for 12 months`（教師制度の文脈で）
- `学生は教師経由で...` `students get ... via teacher`

**例外**：以下は公開して OK
- `初月 50% オフキャンペーン`（HALF50 = 全員対象の公開キャンペーン）
- `年額プランは 2 ヶ月分お得`（年額の通常的な割引）
- 一般的な「学生プランがある」程度の言及（具体的な割引率を伴わない）

### 45.4 OK な場所（露出してよい）

以下のファイルは内部・ゲート済み・オーナー専用なので、教師制度の詳細を書いてよい:

- `/admin/coupons/page.tsx` （オーナー専用ダッシュボード）
- `/for-teachers/page.tsx` （パスワード `kuon` で保護された隠しページ）
- `app/providers.tsx` のコメント（コード内部、ユーザーに見えない）
- `app/page.tsx` の内部コード・コメント（ただし render される文字列はダメ）
- `kuon-rnd-auth-worker/src/*` （バックエンド、ユーザーに見えない）
- `CLAUDE.md` （内部ドキュメント）
- `空音開発/stripe-ids.md` （内部ドキュメント）

### 45.5 「クーポンが適用された」UX の表示ルール

`?coupon=XXX` URL を踏んだユーザー（= 教師から URL を受け取った学生）には、以下のように表示する:
...
❌ NG な表示:
- 🎓 学生クーポン適用中（"学生" の文言で制度がバレる）
- Prelude / Concerto に 30% off × 12 ヶ月（具体的な割引率・期間が漏れる）
- スクショで SNS にバイラルしたら大事故

学生は具体的な金額を Stripe Checkout で見るので、ホームページ側では曖昧な確認だけで十分。

### 45.6 違反時の対応

公開 LP やコードに教師制度の漏洩を発見した場合、Claude は以下を即実行:

1. **何より先に漏洩箇所を Edit で削除**（オーナーへの報告は事後でよい）
2. 全ファイルを grep で再監査（同種漏洩がないか）
3. 修正後にオーナーへ「§45 鉄の掟違反を発見し、X 箇所を修正しました」と報告
4. 該当箇所を CLAUDE.md §45.7 に追記して再発防止

### 45.7 既知の漏洩 → 修正履歴

| 日付 | 漏洩箇所 | 内容 | 修正コミット |
|------|---------|------|------------|
| 2026-04-28 | `app/classical-lp/page.tsx` FAQ「無料で使えますか？」回答 | 6 言語で「教師のクーポンで初月 50% + 12 ヶ月 30%」「Symphony 永久無償」を平文記載 | "クーポン制度の漏洩を全公開 LP から完全削除"（commit hash は実際の push 時に追記） |
| 2026-04-28 | `app/classical-lp/page.tsx` JSON-LD description | "Student discounts available via teacher coupons." を schema.org に記載 → 検索エンジンが取得 | 同上 |
| 2026-04-28 | `app/page.tsx` アクティブクーポンバナー | "🎓 学生クーポン適用中 — Prelude / Concerto に 30% off × 12 ヶ月が自動適用されます" と具体的に表示 | 同上 |

### 45.8 将来 Claude が見落とさないために

新しい LP・FAQ・JSON-LD を作る前に必ずこの §45 を読み返すこと。
特に以下の状況で漏洩リスクが高い:

- 「無料で使えますか？」「料金は？」系の FAQ
- 検索エンジン向けの JSON-LD `description` フィールド
- Hero の「学生応援！」系のキャッチコピー
- 多言語 LP で他の言語版も同時に書き換える際のミス
- アクティブ状態のバナー・トースト・通知文言
- Twitter Card / OGP の description

迷ったら**書かない**。学割について公開で言及する必要が出た場合は、必ずオーナーに確認してから書く。

---

## 46. KUON Music Theory Suite と KUON LAB Suite の分離戦略（2026-04-29 確定）

> このセクションは Kuon が 100 万人スケールで世界市場を取りに行く製品構造を定義する最重要戦略決定の 1 つ。

### 46.1 教育と研究を分けるという根本判断

世界の音楽市場を冷静に分析すると、**音楽理論を学ぶ人口（音大生・社会人学習者・趣味で学ぶ大人）は数千万人規模**、対して**研究レベルでツールを使う層（音楽情報学研究者・上級作曲家・職業音楽家）は数万人規模**。この 1000 倍の差を、同じ製品で扱おうとすると両方が中途半端になる。

教育プロダクトと研究プロダクトでは:
- ユーザーが求める「最初の体験」が違う（教育は優しさ、研究は正確さ）
- 1 セッションあたりの利用時間が違う（教育は 10 分、研究は 2 時間）
- 解約理由が違う（教育は飽き、研究は機能不足）
- バイラル経路が違う（教育は SNS、研究は学術論文の引用）

これらを 1 製品にまとめると、片方の最適化がもう片方を損なう。よって**完全に分離する**。

### 46.2 二つの Suite

**KUON Music Theory Suite**（Prelude プラン ¥780/月〜）
- 楽典基礎から音大卒業レベルまでの完全学習カリキュラム
- マスマーケット向け、毎日使う、世界中の音大生・大人のアマチュアが対象
- URL: `/theory/*` 配下
- §47 のカリキュラム参照

**KUON LAB Suite**（Concerto プラン ¥1,480/月〜）
- 研究・分析グレードのツール群（既存 /classical/lab + 計画中の 10 アプリ）
- プロフェッショナル・研究者向け、論文支援、上級者ロックイン
- URL: `/classical/lab/*` 配下
- 既存 `/classical/lab` をハブとして展開

**既存ブラウザツール群**（Free / Prelude+）
- マスタリング・録音・楽器特化ツール
- 日常制作支援、`/audio-apps` で集約

この 3 層構造で、空音開発は「日常制作 / 学習 / 研究」を全部押さえる**音楽家の生涯プラットフォーム**になる。

### 46.3 隠された巨大市場：「真剣な大人のアマチュア」（2026-04-29 議論で発見）

世界には 30〜60 代の真剣な大人のアマチュア音楽家が膨大に存在する。週末にピアノを弾く、合唱団で歌う、地域オーケストラに参加する、ジャズバーでセッションする、独学で和声を勉強し直す。彼らは**可処分所得があり、教養があり、長期顧客になる**。

そして彼らは現状、音楽教育系アプリに行き場がない。Yousician は子供向け、musictheory.net は退屈、Auralia は古い、YouTube は散漫。

「自分は素人だが、音楽を真剣に愛している。だから自分を子供扱いしないツールが欲しい」というニーズは、世界中で**数千万人規模**。Theory Suite が「余白の知性」（§48 参照）の美意識でこの層を取りに行けば、**音大生市場（数百万人）よりも大きな市場**を独占的に取れる。

つまりプロ・大人音楽家への敬意は、副次的な配慮ではなく**主要収益層の獲得戦略**に直結する。

### 46.4 SNS 独占の正しい戦略

「ショート動画で楽典コンテンツが人気だから空音開発で独占できないか」という問いに対する答え：

**自分でショート動画を量産する戦略は失敗する**（時間が無い）。

**勝てる戦略は逆方向**：「音楽理論コンテンツクリエイターが Kuon を使う状態」を作る。具体的には Theory Suite に**スクリーン録画専用モード**、**アニメーション譜例**、**ワンクリック GIF 出力**、**縦長 9:16 レイアウト対応**を実装し、世界中の音楽教師が動画制作に Kuon を使う環境を作る。動画の片隅に "Made with Kuon" が映る → 視聴者（数千万人の音大生・愛好家）が検索する → 流入。

これは Adobe / Loom / Notion が辿った「コンテンツクリエイターの道具になる」モデル。

### 46.5 競合の地形図と空白市場

**世界に存在しないもの**：
> **「現代的 UI + ブラウザネイティブ + 多言語 + 月額サブスク + 楽典基礎から音大卒業レベルまでの完全カリキュラム + ゲーミフィケーション + SRS + SNS 映え + プロ音楽家への敬意」を 1 つにまとめた製品**

これが Kuon Music Theory Suite が取りに行く空白市場。Auralia / EarMaster は 1990 年代設計、musictheory.net / teoria.com は無料だが進捗管理なし、Yousician / Tonara は子供向け。**現代的 UI + 完全カリキュラム + プロ品位**を全部備えた製品は世界に存在しない。

---

## 47. KUON Music Theory Suite カリキュラム設計（2026-04-29 確定）

### 47.1 基盤教科書：Open Music Theory v2 (OMT) を借用

執筆者: Mark Gotham (ケンブリッジ大), Megan Lavengood (ジョージ・メイソン大), Brian Jarvis, Chelsey Hamm, Bryn Hughes, Kyle Gullings, John Peterson の 7 名。北米の音楽学部で**無料の標準教科書として正式採用**されている。

PDF: `/Users/kotaro/Library/Application Support/Claude/local-agent-mode-sessions/.../uploads/OPEN-MUSIC-THEORY-1688414869.pdf` (1,297 ページ・18MB)

**ライセンス: CC BY-SA 4.0**
- ✅ 自由に引用・改変・商用利用可能
- ✅ サブスク収益化に問題なし
- 📋 義務: クレジット表記 + 派生物も同ライセンスで公開
- 🚫 DRM 禁止

OMT の 10 章 + Anthology + Workbook がそのまま Kuon Theory Suite の骨格になる。1 章 = 1 モジュール、各章のセクションが 1 レッスン。**カリキュラム設計を一から作る必要なし**。

### 47.2 全 16 モジュール構成

OMT の 10 章を骨組みに、Kuon 独自の M0（音楽との出会い）、M11（コールユーブンゲン・聴音）、M12（ソルフェージュ・視唱）、M13-M15 を追加して完全網羅。

**M0 — 音楽との最初の出会い**（Kuon 独自・**Pre-notational**・15 レッスン）

譜面を一切読まずに音楽そのものに触れる入門ブロック。OMT v2 の範囲外。
M1 と内容重複しないよう、あえて記譜法を扱わない。「音楽は気になるが楽典は怖い」層 (世界数千万人規模) を取り込む戦略的入り口。

| # | タイトル | 内容 |
|---|---------|------|
| M0-01 | 音とは何か | 空気の振動・耳の仕組み・周波数と音色の体感 |
...
**M1 — Fundamentals**（OMT Part I、~60 レッスン）
記譜法、音程、音階、三和音、七の和音、ローマ数字、SATB、テクスチャ。

**M2 — Counterpoint and Galant Schemas**（OMT Part II、~35 レッスン）
1-5 種対位法、16 世紀様式、フーガ、グランドベース、ガラント様式。

**M3 — Form**（OMT Part III、~25 レッスン）
楽句、ピリオド、二部・三部・ソナタ・ロンド形式。

**M4 — Diatonic Harmony**（OMT Part IV、~55 レッスン）
カデンツ、V7、属前和音、装飾音、トニカイゼーション、転調。

**M5 — Chromaticism**（OMT Part V、~35 レッスン）
モーダルミクスチャ、ナポリ、増六、共通音、半音的シーケンス、ネオ・リーマン。

**M6 — Jazz**（OMT Part VI、~35 レッスン）
スウィング、コードシンボル、ヴォイシング、ii-V-I、コード・スケール、ブルース。

**M7 — Popular Music**（OMT Part VII、~30 レッスン）
ポップ和声スキーマ、Doo-wop、AABA、Verse-Chorus、モーダル。

**M8 — 20th/21st Century Techniques**（OMT Part VIII、~25 レッスン）
ピッチクラス集合論、整数表記、Forte 番号、教会旋法、集合体。

**M9 — Twelve-Tone Music**（OMT Part IX、~18 レッスン）
12 音技法、行操作、マトリックス、ヴェーベルン分析。

**M10 — Orchestration**（OMT Part X、~15 レッスン）
オーケストレーションの核心原則、音色変化、ピアノからの編曲。

**M11 — Coleübungen と聴音**（Kuon 独自・最大の差別化要素、~90 レッスン）
コールユーブンゲン全 87 番完全インタラクティブ化、音程聴音、和音聴音、カデンツ聴音、旋律聴音、和声聴音、4 声聴音、移調聴音、内的聴覚訓練。

**M12 — ソルフェージュと視唱**（Kuon 独自、~70 レッスン）
階名唱法（移動ド・固定ド両対応）、Pozzoli、Bona、ヒンデミット、コダーイ、多旋律視唱。

**M13 — 音楽史・様式**（Kuon 独自・OMT 軽量補完、~20 レッスン）
スタイル時代概観、主要作曲家、演奏実践（HIP・モダン）、楽器史、記譜法進化。

**M14 — Advanced Theory**（音大上級〜大学院レベル、~30 レッスン）
シェンカー分析、Caplin 形式機能理論、Riemann 機能和声論、微分音、世界の音律、スペクトル分析。

**M15 — 作曲実技**（Kuon 独自・能動的アウトプット、~25 レッスン）
4 声体コラール、弦楽四重奏、リート、ピアノ小品、フリーコンポジション、編曲。

**Bonus — OMT 開発中章**（~25 レッスン）
20 世紀リズム技法、ハイパーメーター、メトリックディソナンス、ドラム記譜、中音和声拡張。

**総レッスン数：概算 553 レッスン**

### 47.3 レッスン 1 個の構成（5 セクション固定テンプレート）

すべてのレッスンを同じテンプレートで量産：

1. **理解（Understand）** — 教科書的説明 200-500 文字（OMT 引用 + Kuon リライト）
2. **観察（Observe）** — 実楽曲（Bach・Mozart 等）からの該当現象、OSMD + Tone.js 統合
3. **実技（Practice）** — music21 ランダム生成のインタラクティブ問題 5-10 問
4. **聴く（Listen）** — 該当現象の聴音 3-5 問、ブラウザ音声合成
5. **理解度確認（Check）** — 10+ フラッシュカード生成 → SRS（FSRS）に投入

これにより 1 レッスンの完成所要時間が「教科書 1 + 譜例 1 + ランダム生成器 + 音声生成 + フラッシュカード」で構造化され、AI 下書き + 校正フローで高速量産可能。

### 47.4 ライセンス遵守事項

CC BY-SA 4.0 の条件:
1. **クレジット表記必須**: 各レッスンに「Based on Open Music Theory v2 by Mark Gotham, Kyle Gullings, Chelsey Hamm, Bryn Hughes, Brian Jarvis, Megan Lavengood, John Peterson, licensed under CC BY-SA 4.0」を表示
2. **派生物も同ライセンス**: Kuon 改変版レッスンも CC BY-SA 4.0 で GitHub 公開（kuon-music-theory-curriculum リポジトリ）
3. 商用 OK、DRM 禁止

**戦略的副作用**: GitHub 公開は学術界からの信頼獲得に直結し、Mark Gotham 教授や OMT コミュニティに「Kuon は OMT を基盤にインタラクティブ実装を行い、学術界に還元している」と発信できる。これは 5 年分のブランド資産になる。

### 47.5 実装フェーズ（推奨順）

**Phase 1（1 ヶ月）**: M0 + M1 の最初の 25 レッスン + スキルツリー UI + SRS エンジン + 日次ストリーク + 診断テスト + ダッシュボード。MVP として公開、初心者層を獲得。

**Phase 2（2-3 ヶ月）**: M4（Diatonic Harmony）+ M11（Coleübungen・聴音）追加で**音大 1-2 年生レベル完成**。日本の音大予備校市場と世界 Conservatory 受験対策市場を取る。コールユーブンゲン完全インタラクティブ化は世界に存在しないので決定的差別化。

**Phase 3（4-5 ヶ月）**: M2 + M3 + M5 + M12 追加で**音大 3-4 年生レベル**。

**Phase 4（6-8 ヶ月）**: M6 + M7 + M8 + M9 + M10 追加で**完全カリキュラム**。

**Phase 5（9-12 ヶ月）**: M13 + M14 + M15 + Bonus で**大学院レベル + 作曲実技**。

### 47.6 IQ180 視点で確定した追加機能（事後報告ではなく事前合意済み）

通常の機能追加とは別に、世界市場制覇のために**最初から組み込む**機能：

1. **診断テスト（プレースメント）**: 15 分テストで M0-M14 の最適スタート地点を判定
2. **模擬入試試験モード**: 東京藝大、ABRSM、AP Music Theory、Juilliard、Berklee、桐朋、国立、Conservatoire de Paris、Berlin Hochschule、韓国芸術総合学校、中央音楽学院（北京）等 12 試験対応（フェーズ 2 以降で順次実装）
3. **教師ダッシュボード**（§44.4 連携）: 教師が自分の生徒のクラス進捗を見る、課題アサイン、教師経由クーポンとの統合
4. **日次ストリーク + 通知**: Resend メール + Push 通知、ストリーク凍結アイテム
5. **SRS（FSRS アルゴリズム）**: フラッシュカードを最適タイミングで再出題
6. **音声入力での視唱・聴音検証**: マイク + ピッチ検出（Pyodide で実装）
7. **埋め込みウィジェット**: iframe 形式で外部サイト・ブログ・YouTube 説明欄に貼れる練習問題
8. **PWA / モバイル対応**: ホーム追加可能、Service Worker でオフライン、Push 通知
9. **SNS 用シェア生成**: Canvas API で 1080x1920 縦長画像/動画自動合成、Instagram/TikTok ワンクリック投稿
10. **AI チューター**（軽量版）: 事前定義 Q&A テンプレートマッピング + 将来は WebGPU 上の小型 LLM
11. **教育機関向け B2B プラン**: 「Kuon for Schools」既存 `/for-schools` ページ拡張、年額 50-200 万円、SSO 対応
12. **OMT コミュニティへの貢献**: GitHub kuon-music-theory-curriculum 公開で学術界からの信頼獲得

### 47.7 確定事項のまとめ

- ブランド名: **KUON Music Theory Suite**
- 配置: `/theory/*`（独立ルート、`/classical/theory` ではない）
- 価格: Prelude (¥780/月) から無制限利用
- 多言語: 日本語 + 英語で MVP、フェーズ 2 でドイツ語・スペイン語、フェーズ 3 で韓国語・ポルトガル語
- ソルフェージュ: 移動ド・固定ド両対応、デフォルトはユーザーの言語設定で選択（日本語 → 固定ド、欧州語 → 移動ド）
- 機能名併記: 国際的な学習体験優先で**ローマ数字のみ**を基本、各レッスン冒頭に説明
- 模擬試験スコープ: フェーズ 2 で東京藝大 + ABRSM をまず実装（日本市場 + 英連邦市場）
- 教師ダッシュボード: フェーズ 3 で実装
- コスト: 完全ゼロ運用可能。フェーズ 2 で Salamander Piano（CC BY 3.0）を R2 ホストして音質強化（月数百円）

---

## 48. マイページ統合ハブと「余白の知性」美学（2026-04-29 確定）

> Kuon が「いいアプリ」で終わるか「世代を超えた音楽家の家」になるかを分ける根本設計。

### 48.1 統合の必然性

ユーザーの心の中で「マイページ」は単数形。「俺の場所」が 1 つあって欲しい。それが Theory・LAB・Subscription・Favorites と分散すると Kuon が「複数の小さなアプリの寄せ集め」に見える。逆に統合すると**「Kuon = 私の音楽の家」というブランド体験**が確立する（Notion / Linear / GitHub のハブモデル）。

決定: **`/mypage` を空音開発全体のセントラルハブとする**。`/theory/dashboard` のような独立進捗ページは作らない。

### 48.2 ハブ・アンド・スポーク構造

`/mypage` は**ハブ**として全サービスの「今日の状況」を一覧表示、深掘りは専用ページに遷移：

```
/mypage                          ← 統合ハブ（全サービスの集約）
/mypage/profile                  プロフィール編集
/mypage/subscription             Stripe Customer Portal 入口
/mypage/achievements             バッジ全覧（横断）
/mypage/notifications            通知設定

/theory                          Theory Suite 入口（LP 兼ハブ）
/theory/tree                     スキルツリー全体（深掘り版）
/theory/practice                 ランダム練習モード
/theory/exam                     模擬試験ハブ
/theory/m{N}/c{M}/l{K}           各レッスン

/classical/lab                   分析環境（既存）
/classical/lab/history           分析履歴専用ページ（今後追加）
```

`/mypage` の構成：
1. **プロフィール + プラン状況**（既存）
2. **今日のスナップショット**: 連続学習日数、今日のレッスン進捗、今日の練習時間
3. **サマリーカード群**:
   - 🎓 Theory（楽典学習）→ `/theory/tree` へリンク
   - 🧪 Classical Lab（分析）→ `/classical/lab/history` へリンク
   - ⭐ お気に入りアプリ（既存 MyAppsSection）
   - 🏆 達成バッジ → `/mypage/achievements` へリンク
4. **サブスク状況**（既存）と**通知設定**

### 48.3 美意識「余白の知性」（Negative Space Intelligence）

100 万人スケールでブランドが成立する条件。日本の能・水墨画、北欧ミニマリズム、Apple Human Interface Guidelines に共通する原則：**情報を詰め込まないことそのものが、ユーザーへの敬意になる**。

具体的な美意識として以下を採用：

**第一・タイポグラフィの威厳**: Shippori Mincho（明朝）を主見出し、Helvetica Neue / Inter をデータ表示。「楽譜の上品な余白」と「現代音楽家の理性」の両方を視覚化。Mincho は 100 年前の音楽書籍の誇りを継承する記号、Helvetica は 60 年前の Swiss モダニズムの冷静さの記号。

**第二・数値表現の抑制**: 「12 日連続！🔥🔥🔥」ではなく控えめな「12 日継続」。バッジは爆発でなく静かなフェードイン。色は鮮やかな赤緑黄ではなく深みのある藍・墨・茶・金。**毎日見ても疲れない**設計。

**第三・装飾としての楽譜記号**: SMuFL（Bravura フォント）の音楽記号を要所に配置。区切り線の代わりに譜表線、ローディングの代わりに音符の動き。

**第四・時刻によって変わる空気**: 早朝は「おはようございます。今日も練習を始めましょうか」、深夜は「夜遅くまでお疲れさまです。短い復習だけでも」。ダークモード自動適用。

**第五・人格を持った言葉遣い**: 「Great Job!」のような褒め称えはしない。事実を淡々と。日本語は「音大時代の先生から手紙をもらった気持ち」になる文体。

**第六・キャラクター・マスコットを使わない**: Duolingo のフクロウのような子供向け要素を完全排除。これは大人音楽家への敬意の核心。

### 48.4 3 モード適応設計

「子供にも大人にも」という両立はユーザーが自分で深さを選べる仕組みで解決：

**Apprentice モード（学習者向け）** — ストリーク・バッジ・進捗バーが大きく見える。「次のレッスンへ」が視覚的に強調。励ましメッセージあり。初心者・若年層に最適。

**Standard モード（中級・一般）** — 進捗とデータがバランス良く。装飾控えめ。励ましメッセージは控えめ。

**Master モード（プロ・上級）** — ストリーク・バッジは小さく/非表示。データのみ大きく。専門家向け数値表示。励ましメッセージなし。プロが毎日開いても煩わしくない静寂。

ユーザーは登録時の診断テスト結果から自動選択され、いつでも変更可能。同じプラットフォームで初心者は励まされ、プロは敬意を払われる。

### 48.5 Pyodide ウォームアップの隠れた価値

ユーザーが朝に `/mypage` を開いた瞬間、バックグラウンドで Pyodide + music21 をプリロード開始。「今日のレッスンをやろう」とクリックする頃には既に準備完了している状態を作れる。これは `/mypage` を経由せず直接レッスン URL に飛ぶ動線では成立しない。**ハブを持つことで体験が滑らかになる**。

### 48.6 隠れた巨大市場への対応

§46.3 で述べた「真剣な大人のアマチュア音楽家」（数千万人規模）を取りに行くため、マイページが「大人も使える美意識」を体現することが必須。具体的には:

- マスコット・キャラクター禁止
- 派手な色・爆発エフェクト禁止
- 子供っぽいイラスト禁止
- 「Awesome!!!」「Great Job!」禁止
- ゲーム的演出禁止（Master モード時）

代わりに:
- Apple Music / Linear / Notion / iA Writer 級の品位
- Steinway コンサートホール website のような威厳
- 作曲家の名言を ambient text として薄く流す
- 静寂と余白を大切にする

### 48.7 開発工程の変更

マイページは「機能の集積場」ではなく**空音開発のブランドそのものを体現する場所**。よって：

1. マイページ専用に 2-3 週間の集中設計期間を取る
2. 3 モード（Apprentice/Standard/Master）を最初から実装
3. ヘッダー・フッター・余白・色彩・タイポグラフィを**ブランドガイドライン**としてドキュメント化
4. プロのテスター（音大教授・現役ピアニスト・指揮者）に最低 3 人見てもらう
5. 6 言語の翻訳を機械翻訳直訳でなく、各言語の音楽教育文化に合わせた敬意表現に整える

### 48.8 既存マイページへの影響

現 `/mypage` は MyAppsSection（お気に入り）とプラン状況が中心。これに**新しい 2 枚のカード**（Theory 進捗 / Classical Lab 履歴）を追加し、全体を「余白の知性」基準に再デザインする。既存ユーザーが「あれ、進化した」と感じる程度の変化を目指す。動いている機能はすべてそのまま生き残る。

---

## 49. 創業者の創作哲学とフィードバック設計（2026-04-29 確定）

> このセクションは KUON Music Theory Suite の全レッスン、AI チューターの全応答、楽譜・聴音・視唱フィードバック、UI 文言、SNS シェア生成、エラーメッセージに至るまで、**全ての判断の最終基準**として機能する。違反は §45 と同等の重大度で扱う。

### 49.1 背景

朝比奈幸太郎の創作ベースには前衛音楽とインプロビゼーションがある。これは趣味嗜好ではなく、創作者としての根幹哲学である。Kuon の Theory Suite はこの哲学の上に成立しており、Yousician / Auralia / EarMaster / musictheory.net など全競合と決定的に異なるブランド DNA となる。

### 49.2 核心原則

**「正解」と「不正解」の二項対立を排する。**

Bach がどう書いたか、Mozart がどう解いたか、教科書がどう推奨するか — それらは全て「**一つの選択**」であり、唯一の正解ではない。学習者の選択もまた、成立する音楽的選択であり得る。

音楽は規則の従順な再現ではない。前衛芸術・即興演奏が示してきたように、規則は「土台」であり「天井」ではない。Kuon の Theory Suite は規則を体系的に教えるが、規則の外に立つ自由を最初から尊重する。

### 49.3 AI チューター・フィードバックの言葉遣い（許可）

学習者の入力を受けたとき、以下のニュアンスで応答する：

- 「**定番の答えは X です**」（教科書的回答の提示）
- 「**入試では Y が期待されます**」（試験対策的回答）
- 「**Bach は Z を選びました — その理由は…**」（歴史的選択の提示）
- 「**あなたの選択 W も成立する音楽です — 理由は…**」（学習者の選択の肯定 + 分析）
- 「**この場面での選択肢は複数あります**」（複数解の前提）
- 「**A 案・B 案・C 案を比較します**」（並列的提示）
...
- 学習者の選択を否定する一切の表現
- 「これしかない」「正解は X だけ」という単一解答主義
- 採点における「点数 / 満点」式の機械的評価（達成感のための数値化は OK だが、唯一解前提の減点は NG）
- 学習者を「初心者だから簡単に」と上から見下ろす表現
...
| 適用対象 | 具体例 |
|---|---|
| 全 553 レッスンのフィードバック設計 | 「Bach は V → I を選んだ」「あなたの V → vi も成立する」 |
| AI チューターの全応答 | 質問 → 教科書解 + 代替解 + 学習者選択肯定 |
| 楽譜入力時の声部進行検証 | 「連続 5 度を検出 — Bach はこの場面でどう避けたか / あえて使う作曲家もいる」 |
| 聴音・視唱の判定 | 「目標音 → C、あなた → C+10 cent — 平均律と純正律で異なる」 |
| エラーメッセージ・トースト・通知文言 | 「保存失敗」(NG: 「失敗しました!」) |
| SNS シェア生成時の自動キャプション | 「Bach Chorale 152 を分析しました」(NG: 「正解しました!」) |
| クォータ・サブスク UI | 「今月の使用回数 X / Y」(NG: 「制限を超えました!」) |

### 49.6 体系性と自由の両立

**規則は教える。規則の外に立つ自由も同時に教える。**

「カデンツとは V → I である」は教える。しかし同時に「Bach は BWV 269 で V → vi を選んだ。それは偽終止と呼ばれ、聴き手の予測を裏切ることで物語を続ける効果がある」と並列で示す。

学習者が「I → vi → V → I」のような変則進行を入力したとき、「不正解」とせず、「定番から外れた選択ですが、ジャズスタンダードでは頻出します。Bill Evans の "Waltz for Debby" 冒頭がこの動きです」と応答する。

### 49.7 哲学のブランド DNA としての位置

この哲学は単なる UX ガイドラインではなく、**Kuon の競合不可能性の核心**である：

- Yousician は「正解音」を判定するゲームエンジン → §49 と矛盾
- Auralia / EarMaster は「答え合わせ」式 → §49 と矛盾
- musictheory.net は教科書的回答のみ → §49 と部分矛盾
- Duolingo 式は「Awesome!」「Wrong!」 → §49 と完全矛盾

**§49 を貫徹できるのは、創業者自身が前衛・即興畑であり、かつ AI チューターの応答テンプレートを直接設計できる Kuon だけ**である。これが「世界最高峰の音楽理論プラットフォーム」を名乗る根拠の一部となる。

### 49.8 違反時の対応

公開機能・LP・レッスン本文に §49 違反を発見した場合、Claude は §45 違反と同等の優先度で即座に修正する：

1. 違反箇所を直ちに Edit で削除・書き換え
2. オーナーへ「§49 違反を発見し X 箇所を修正しました」と報告
3. 該当箇所を §49.9 に追記して再発防止
...
## 50. KUON Music Theory Suite 開発状況メモ — 必読参照ファイル

> **Theory Suite の開発を進める / 再開する際は、必ず以下のファイルを最初に読むこと。**

**ファイルパス:** `空音開発/theory-development-state.md`

このファイルには以下が網羅されている:
- 公開済みレッスン一覧 (現在 3/553)
- 確立した 3 レイヤー構造 (Story / Living Score / Memory) のテンプレート
- 共通サブコンポーネント (LayerLabel, PyodideBadge, ModeButton, FlashcardItem, RelatedTool 等)
- Pyodide / Bravura font / Web Audio の確立済みロードパターン
- デザイントークン (色彩・フォント・SMuFL コードポイント)
- レッスン量産プロセス (1 レッスン約 30-60 分)
- 残り 13 モジュールの量産優先順位 (議論待ち)
- 重要な意思決定の履歴 (12 項目・2026-04-29 確定)
- 開発再開時のチェックリスト

### 50.1 開発再開時の手順

オーナーが「Theory Suite の開発を再開」と指示したら、Claude は必ず以下を実行する:

1. `空音開発/theory-development-state.md` を読む
2. CLAUDE.md §45 (教師経由学生クーポン機密)、§47 (カリキュラム)、§48 (余白の知性)、§49 (創業者哲学) を再確認
3. `git log --oneline -20` で直近のコミットを確認
4. オーナーに以下を質問:
   - 次に作るレッスンの優先順位 (M0 残り / M1 残り / M11 / ハイブリッド)
   - 公開済みレッスンへの修正点
   - 新しい戦略的決定の有無
...
- §49 フィードバック (「定番の答えは X」「あなたの選択も成立する」式)
- §48 美学 (マスコット・感嘆符・励まし禁止、明朝主体)
- 6 言語対応 (ja / en / es / ko / pt / de) — 機械翻訳直訳ではなく文化適応
- Layer 1 は Pyodide ロード中に完全に読める設計
- 関連ツールリンクは必ず 3 つ含める (Kuon の他アプリへの導線)
- sitemap.ts への登録を忘れない
- ハブの `availableLessons` 配列に追加を忘れない

### 50.3 レッスン量産の物理スループット

テンプレートが完成しているため 1 レッスン約 30-60 分。週 10 レッスンのペースで進めれば、550 レッスン完成まで約 55 週間 (約 1 年)。これは現実的な見通し。

ただし量産だけが目的ではない。M11 (コールユーブンゲン世界初インタラクティブ化) のような「世界に存在しない決定的差別化要素」は時間をかけて深掘りする価値がある。

---

## 51. 西洋五線譜記述の絶対正確性 — 鉄の掟 (2026-04-29 確定)

> このセクションは §45・§49 と同等の最重要ルール。違反は即時修正対象。

### 51.1 鉄の掟の本文

**Theory Suite 内で五線譜を表示する場合、Unicode 文字 (𝄞 ♩ ♪ ♫ 等) や Bravura グリフ単独 では絶対に表現してはならない。**

代わりに、必ず以下のいずれかで描画する:

1. **SVG による真の五線譜** — 5 本の線、適切な音部記号 (Bravura SMuFL)、加線 (ledger lines)、楕円形の符頭 (rotate -20deg)、必要なら符尾・連桁
2. **OpenSheetMusicDisplay (OSMD) による MusicXML レンダリング** — 楽曲全体を扱う場合
3. **Bravura SMuFL 単独グリフ** — 音部記号や臨時記号を文章中に挿入する場合のみ (旋律や和音には絶対に使わない)

### 51.2 違反例 (絶対に避ける)

```tsx
// ❌ 絶対 NG: Unicode 文字で旋律を表現
representation: '𝄞 ♩ ♩ ♩'

// ❌ 絶対 NG: 5 線も加線もない、ただの並び
<div>♩ ♩ ♩</div>

// ❌ NG: 音部記号だけ使って音高を Bravura グリフ単独で並べる
<text fontFamily="Bravura">𝄞 𝅗𝅥 𝅗𝅥 𝅗𝅥</text>
```
...
function StaffMiniSvg() {
  return (
    <svg viewBox="0 0 220 80">
      {/* 5 staff lines */}
      {[0,1,2,3,4].map(i => (
        <line key={i} x1={45} x2={W-12} y1={STAFF_TOP + i*GAP} y2={STAFF_TOP + i*GAP}
              stroke={STAFF_LINE_COLOR} strokeWidth={1}/>
      ))}
      {/* Treble clef (Bravura SMuFL U+E050) */}
      <text fontFamily="Bravura" fontSize={GAP*4}>{String.fromCodePoint(0xE050)}</text>
      {/* Notes with proper diatonic positioning + ledger lines for C4 */}
      <ellipse cx={cx} cy={midiToY(60)} rx={GAP*0.85} ry={GAP*0.6}
               transform={`rotate(-20 ${cx} ${midiToY(60)})`}/>
    </svg>
  );
}
```
...
| 適用対象 | 違反すると |
|---|---|
| Theory Suite 全 553 レッスン | レッスンの教育価値が崩壊 |
| LP の楽譜サンプル | ブランドの信頼性が崩壊 |
| ホームページの装飾 | 楽典スクール名乗りが崩壊 |
| OG 画像・Twitter Card | SEO 評価が崩壊 |
| マイページの表示 | プロ音楽家の信頼が崩壊 |

### 51.5 なぜこのルールがあるのか

世界中の音楽家・プロ・音大教授・100 万人を目指すアプリとして、五線譜の正確な描画は譲れない一線である。Unicode 文字や Bravura グリフ単独では:

- 線がない → 音高が読めない
- 加線がない → 中央ハ (middle C) を区別できない
- 符頭の傾きがない → 楽典の正書法に違反
- 5 線でない場合がある → そもそも記譜と認識されない

これは「正解は一つ」を排する §49 とは独立した、**技術的正確性の最低ライン**である。哲学的な多様性は思想の話、楽譜の描画は工芸の話 — 工芸の精度に妥協はない。

### 51.6 既知の違反 → 修正履歴

| 日付 | 違反箇所 | 内容 | 修正 |
|------|---------|------|------|
| 2026-04-29 | M1-01 Living Score 西洋五線譜カード | `representation: '𝄞 ♩ ♩ ♩'` で 4 つの音符と clef を Unicode で並べていた。実際は do-mi-sol の 3 音メロディーであるべき。線も加線もなかった | `StaffMiniSvg` コンポーネントを追加し、5 線・treble clef・C4 加線・E4 第 1 線・G4 第 2 線を全て SVG で正確に描画 |

### 51.7 違反時の対応

公開機能・LP・レッスン本文に §51 違反を発見した場合、Claude は §45・§49 違反と同等の最優先で:

1. 違反箇所を直ちに削除し、正確な SVG / OSMD 描画に置換
2. オーナーへ「§51 違反を発見し X 箇所を修正しました」と報告
3. §51.6 に追記して再発防止
